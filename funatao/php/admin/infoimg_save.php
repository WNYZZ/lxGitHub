...
if($action == 'add')
{
	...
	if(!isset($en_title))  $en_title = '';
	if(!isset($en_description))  $en_title = '';
	if(!isset($en_content))  $en_title = '';
	if(!isset($big_picurl))  $en_title = '';
	...
	if($remote == 'true')
	{
		...
		$en_content = GetContFile($en_content);
	}
	...
	if($autodesc == 'true')
	{
		...
		$en_descstr  = ClearHtml($en_content);
		$description = ReStrLen($descstr, $autodescsize);
		$en_description = ReStrLen($en_descstr, $autodescsize);
    ...
    if($autopage == 'true')
    {
        ...
        $en_content = ContAutoPage($en_content, $autopagesize*1024);
    }
...
while($row = $dosql->GetArray())
	...
	$sql = "INSERT INTO `$tbname` (siteid, classid, parentid, parentstr, mainid, mainpid, mainpstr, title, colorval, boldval, flag, source, author, linkurl, keywords, description, content, picurl, picarr, orderid, hits, posttime,en_title,en_description,en_content,big_picurl, checkinfo {$fieldname}) VALUES ('$cfg_siteid', '$classid', '$parentid', '$parentstr', '$mainid', '$mainpid', '$mainpstr', '$title', '$colorval', '$boldval', '$flag', '$source', '$author', '$linkurl', '$keywords', '$description', '$content', '$picurl', '$picarr', '$orderid', '$hits', '$posttime','$en_title','$en_description','$en_content','$big_picurl', '$checkinfo' {$fieldvalue})";
...
else if($action == 'update')
{
	...
	if($autodesc == 'true')
	{
		...
		$descstr     = ClearHtml($content);
		$en_descstr     = ClearHtml($en_descstr);
		$description = ReStrLen($descstr, $autodescsize);
		$en_description = ReStrLen($en_descstr, $autodescsize);
	}
